...
const float HUMAN_SPEED = 4.6f;
const float ZOMBIE_SPEED = 2.3f;
const float PLAYER_SPEED = 8.8f;
...
MainGame::MainGame()  :
    ...
    //initializer list most stuff set 0 and screen sizes to desired values
{
    // Empty
}
...
void MainGame::run()
{
    ...
    MessageBox(NULL, "Zombie Exterminator", "Welcome", NULL);

    MessageBox(NULL, " Use: W,A,S,D to move. \n MouseClick1 to Shoot \n Keys: 1-4 to change weapon", "Game Keys", NULL);
...
void MainGame::initSystems()
{
    ...
    // Create our window
    _window.create("Zombie Exterminator", _screenWidth, _screenHeight, 0);
...
void MainGame::initLevel()
{
    ...
    // Make guns for player by calling constructor whit values;
    _player->addGun(new Gun("GMGun", 10, 1, 0.1f, 100, 10.1f));
    _player->addGun(new Gun("Shotgun", 30, 20, 1.0f, 100, 20.0f));
    _player->addGun(new Gun("Magnum", 2, 5, 0.6f, 10, 30.0f));
    _player->addGun(new Gun("MP5", 15, 40, 0.4f, 10, 5.5f));
    _player->addGun(new Gun("Umlg",3,50,10,300,40.0f));
...
void MainGame::updateAgents(float deltaTime)
{
    ...
    for (size_t i = 0; i < _zombies.size(); i++)
    {
        ...
        if (_zombies[i]->collideWithAgent(_player))
        {
            ...
            MessageBox(NULL, "You got eaten by a zombie", "GAME OVER", NULL);
            SDL_Quit();
            exit(69);
        }
    }
...
void MainGame::checkVictory()
{
    ...
    if (_zombies.empty())
    {
        ...
        MessageBox(NULL, " You win! \n You killed all the zombies.", "YOU WIN", NULL);
